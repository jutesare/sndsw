...
def alignPlotPrint():
    print("Will run SciFi alignment, plot the residuals and print the resulting parameters.")
    minimizeAlignScifi()
    printScifi_residuals()
    printScifiAlignment()
    print("Done.")

def minimizeAlignScifi(first=False,level=2,migrad=False):
    ...
    if level == 2:
      ...
      for s in range(1,nScifi+1): # station
          for o in range(2): # orientation (0=horizontal, 1=vertical)
      ...
      for s in range(1,nScifi+1): # station
          for a in range(3): # angles (0=phi, 1=psi, 2=theta)
              if s==3:
                  gMinuit.FixParameter(p) # fix the orientation of one plane, again following Simona's note
              p+=1
        
      # see here what was in the script before I adjusted it
      """
      for s in range(1,nScifi+1):
        for o in range(2):
             for m in range(nMats):
